...
const NetworkComponent = ({ connectedUsers, requests, suggestions, user, csrfToken }) => {
  ...
  const requestUrl = basePath + urls.request.getByUser.replace(":user", session?.user._id)
  useEffect(() => {
    (async () => {
      const res = await axios.get(connectionsUrl, {
        headers: {
      ...
      <main className="networkContainer">
        ...
        <div className="rightNetworkContainer content">
          <div className="card left-group">
            ...
            {
              connectedUser?.length > 0 ? connectedUser?.map(eachUser => {
