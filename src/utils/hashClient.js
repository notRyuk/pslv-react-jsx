export default class Hash {
    ...
    static async compare(hash, password) {
        ...
        const hashBuffer = new Uint8Array(atob(hash).split('').map(char => char.charCodeAt(0)));
        const passwordHashBuffer = new Uint8Array(atob(passwordHash).split('').map(char => char.charCodeAt(0)));
